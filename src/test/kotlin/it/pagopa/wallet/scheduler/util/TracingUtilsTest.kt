package it.pagopa.wallet.scheduler.util
...
import it.pagopa.wallet.scheduler.common.tracing.TracingUtils.Companion.TRACEPARENT
import java.util.*
...
class TracingUtilsTest {
    ...
    companion object {
        fun getMock(): TracingUtils {
            val mockedTraceInfo = TracingInfo(UUID.randomUUID().toString(), "", "")
            ...
            Mockito.`when`(mockedTracingUtils.traceMonoQueue<Any>(any(), any())).thenAnswer {
                invocation ->
                return@thenAnswer (invocation.getArgument<Any>(1) as (TracingInfo) -> Mono<*>)
